...
Maverick.Application.prototype.dispatch = function(configDir, port, host) {
  var me = this;
  ...
  this.appDir = nodePath.resolve(this.configDir, '../'),
  this.moduleDir = nodePath.resolve(__dirname);

  this.requireDirs = [nodePath.join(this.moduleDir, "view_renderers"), nodePath.join(this.appDir, "app/controllers"), nodePath.join(this.appDir, "app/helpers")];

  this.loadDependencies();

  console.log("Dispatching Maverick Application");
  console.log("    listening on port " + this.port.toString() + " at " + this.host + "\n");

  var server = new Maverick.Server();
  server.start(this.port, this.host, function(request) {
...
Maverick.Application.prototype.loadDependencies = function() {
  ...
  require(nodePath.join(this.configDir, "environments/" + this.environment));
